...
<details><summary>
API 임계값 확인 및 부하 테스트 수행</summary>
백엔드 개발자로서 대용량 트래픽을 견딜 수 있는 코드를 작성하기 위해, 현재 구현한 API의 임계값을 확인하고자 부하 테스트를 진행했습니다. 현재는 1회 처리량이 적어 문제가 없지만, 향후 트래픽이 증가할 것을 대비해 테스트를 수행했습니다.

### 부하 테스트 툴 선정

JMeter를 선택한 이유는 다음과 같습니다.

- 사용자가 많아 다양한 레퍼런스 확보 가능

- Java 기반 오픈소스이므로 Java로 작성된 API 테스트에 적합


### 테스트 시나리오

***블랙 프라이데이를 맞이하여 전 품목 최대 55% 할인 이벤트로 사용자가 몰린 상황을 가정***

- 500명 동시 접속 후 로그인 및 상품 전체 조회 실행

- 5분 동안 지속적인 부하 발생

- 오류율 1% 미만 유지

- 모든 요청 응답 시간 5초 이내 보장


### 테스트 환경 구축

<img width="369" alt="스크린샷 2025-02-20 오후 3 43 49" src="https://github.com/user-attachments/assets/8878ea2a-83a2-428b-b9aa-935d7ff85423" />

- Thread Group: 500명의 동시 접속자 설정

- Sampler: 로그인 및 상품 전체 조회 요청 구성

- Listener: 응답 데이터 수집 및 분석

- Constant Timer: 요청 간 대기 시간 2초를 설정해 자연스러운 트래픽 시뮬레이션


### 테스트 진행 중 발생한 문제 및 해결 과정

**1. 로그인 요청 403 오류 발생**

로그인 API는 인증 및 인가가 필요 없는 구조임에도 불구하고 403 오류가 발생했습니다. 조사 결과, 요청의 Content-Type이 명시되지 않으면 오류가 발생할 수 있다는 사실을 확인했습니다. HTTP Header Manager를 추가하고 Content-Type: application/json 설정했습니다.

**2. 로그인 요청 400 오류 발생**

400 오류는 클라이언트 요청에 문제가 있어 서버가 처리할 수 없는 상황을 의미합니다. IDEA에서 로그를 확인한 결과, 가입된 계정이 없어 로그인 요청이 실패한 것이 원인이었습니다. 로그인 전 회원가입 요청을 추가했습니다.

**3. 회원가입 중복 문제**

<img width="1126" alt="스크린샷 2025-02-20 오후 3 49 02" src="https://github.com/user-attachments/assets/b454b7a8-5bbb-4403-9402-1590912963d2" />

500명의 사용자가 동시에 회원가입을 진행하기 위해서는 각기 다른 계정이 필요했기 때문에 JMeter의 내장 변수를 활용하여 각 스레드마다 고유한 사용자 계정 생성했습니다.

**4. 상품 전체 조회 요청에서 로그인한 사용자와 비로그인 사용자의 응답이 동일한 문제**

![image](https://github.com/user-attachments/assets/ccfe911a-63d4-482e-976e-c130233bbd00)
<img width="1136" alt="스크린샷 2025-02-20 오후 6 08 19" src="https://github.com/user-attachments/assets/291707a1-c541-4d2a-8768-035819cb1b7a" />

로그인한 사용자와 로그인하지 않은 사용자에게 제공되는 상품 정보가 다름에도 불구하고, 테스트에서는 로그인을 하지 않은 상태로만 요청이 전송되고 있었습니다. 이는 로그인 후 받은 토큰을 상품 조회 요청에 포함하지 않았기 때문입니다. Regular Expression Extractor를 사용하여 로그인 응답에서 토큰을 추출하고, 해당 토큰을 상품 조회 요청의 헤더에 추가했습니다.

### 테스트 결과 및 개선 방향

![image](https://github.com/user-attachments/assets/34f6ebad-7d65-4ee9-8b83-753cd05bee13)

테스트 결과, 예상했던 시나리오와 달리 오류율이 1% 이상이었습니다. 모든 오류는 응답 시간 초과로 인한 것이었으며, 이는 현재 코드의 성능이 충분하지 않음을 의미합니다. 이번 테스트를 통해 API의 한계를 파악하고, 대용량 트래픽에 대비한 성능 개선이 필요함을 확인했습니다. 앞으로 추가적인 최적화를 진행하며, 안정적인 서비스 운영을 위한 노력을 지속할 예정입니다.
